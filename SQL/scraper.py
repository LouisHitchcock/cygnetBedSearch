...
DATABASE_FILE = 'bed_availability.db'
...
def save_data(male_wards, female_wards):
    ...
    for ward, info in male_wards.items():
        cursor.execute('''
            INSERT INTO bed_availability (ward, beds, purpose, gender)
            VALUES (?, ?, ?, ?)
        ''', (ward, info['beds'], info['purpose'], 'Male'))

    for ward, info in female_wards.items():
        cursor.execute('''
            INSERT INTO bed_availability (ward, beds, purpose, gender)
            VALUES (?, ?, ?, ?)
        ''', (ward, info['beds'], info['purpose'], 'Female'))
